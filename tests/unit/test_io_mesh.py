"""Unit tests for the IWFM mesh + stratigraphy ASCII I/O handlers."""
...
from pyiwfm.io.preprocessor.mesh import _is_comment_line, _strip_comment  # noqa: E402
...
class TestCanonicalFormat:
    """Tests for IWFM-conventional format produced by the canonical writers.

    These pin down behaviour shared with the rest of the writer ecosystem
    (e.g. PreProcessorWriter delegates here): always-emitted FACTXY/FACTEL
    factor lines, AQT_L*/AQF_L* column legend, ASCII-art title block.
    """

    def test_nodes_emit_factxy_default_factor(self, tmp_path: Path) -> None:
        out = tmp_path / "nodes.dat"
        write_nodes(out, {1: Node(id=1, x=0.0, y=0.0)})
        content = out.read_text()
        assert "FACTXY" in content
        assert "1.000000" in content  # default factor
        assert "IWFM NODAL COORDINATES FILE" in content
        assert "Generated by pyiwfm" in content

    def test_nodes_factor_round_trip(self, tmp_path: Path) -> None:
        out = tmp_path / "nodes.dat"
        nodes = {1: Node(id=1, x=100.0, y=200.0)}
        write_nodes(out, nodes, factor=0.3048)
        # The reader applies the factor on read; we wrote x=100 with factor=0.3048,
        # so read_nodes should report x = 100 * 0.3048 = 30.48
        nodes_back = read_nodes(out)
        assert nodes_back[1].x == pytest.approx(100.0 * 0.3048)
        assert nodes_back[1].y == pytest.approx(200.0 * 0.3048)
        # And the factor line is present
        assert "0.304800" in out.read_text()

    def test_elements_default_subregion_names(self, tmp_path: Path) -> None:
        out = tmp_path / "elements.dat"
        elements = {
            1: Element(id=1, vertices=(1, 2, 3, 4), subregion=1),
            2: Element(id=2, vertices=(2, 3, 4, 5), subregion=2),
        }
        write_elements(out, elements)
        content = out.read_text()
        assert "Region_1" in content
        assert "Region_2" in content

    def test_elements_custom_subregion_names_round_trip(self, tmp_path: Path) -> None:
        out = tmp_path / "elements.dat"
        elements = {
            1: Element(id=1, vertices=(1, 2, 3, 4), subregion=1),
            2: Element(id=2, vertices=(2, 3, 4, 5), subregion=2),
        }
        write_elements(out, elements, subregion_names={1: "North_Region", 2: "South_Region"})
        elements_back, n_sr, names_back = read_elements(out)
        assert n_sr == 2
        assert "North_Region" in names_back.values()
        assert "South_Region" in names_back.values()
        # Element data still round-trips
        assert elements_back[1].subregion == 1
        assert elements_back[2].subregion == 2

    def test_elements_n_subregions_inferred(self, tmp_path: Path) -> None:
        """Omitting n_subregions infers the count from the element subregions."""
        out = tmp_path / "elements.dat"
        elements = {
            1: Element(id=1, vertices=(1, 2, 3), subregion=3),
            2: Element(id=2, vertices=(1, 3, 4), subregion=7),
            3: Element(id=3, vertices=(2, 3, 4), subregion=3),
        }
        write_elements(out, elements)
        _, n_sr, _ = read_elements(out)
        assert n_sr == 2  # two distinct subregion IDs (3 and 7)

    def test_stratigraphy_emits_factel_default_factor(self, tmp_path: Path) -> None:
        out = tmp_path / "strat.dat"
        strat = Stratigraphy(
            n_layers=1,
            n_nodes=1,
            gs_elev=np.array([100.0]),
            top_elev=np.array([[90.0]]),
            bottom_elev=np.array([[50.0]]),
            active_node=np.array([[True]]),
        )
        ...
        assert "AQT_L1" in content
        assert "AQF_L1" in content
        assert "IWFM STRATIGRAPHY DATA FILE" in content

    def test_stratigraphy_column_legend_per_layer(self, tmp_path: Path) -> None:
        """The column legend names every layer."""
        out = tmp_path / "strat.dat"
        n_layers = 4
        gs = np.array([100.0])
        top = np.array([[100.0 - 20 * i for i in range(n_layers)]])
        bot = np.array([[80.0 - 20 * i for i in range(n_layers)]])
        strat = Stratigraphy(
            n_layers=n_layers,
            n_nodes=1,
            gs_elev=gs,
            top_elev=top,
            bottom_elev=bot,
            active_node=np.ones((1, n_layers), dtype=bool),
        )
        write_stratigraphy(out, strat)
        content = out.read_text()
        for k in range(1, n_layers + 1):
            assert f"AQT_L{k}" in content
            assert f"AQF_L{k}" in content
