...
class IWFMStratigraphy:
    ...
    def __init__(self, nl, fact, stratigraphy):
        ...
        if not isinstance(stratigraphy, list) and not all(
            [isinstance(item, NodeStratigraphy) for item in stratigraphy]
        ):
        ...
        layer_check = np.where(
            np.array([len(ns.layer_thicknesses) for ns in stratigraphy]) != 2 * self.nl
        )[0]

        if len(layer_check) > 0:
            raise ValueError(
                "stratigraphy provided does not match the number of layers provided"
            )
    ...
    def to_dataframe(self):
        ...
        df = pd.concat(
            [
                pd.DataFrame(node_ids),
                pd.DataFrame(gse),
                pd.DataFrame(layer_thicknesses),
            ],
            axis=1,
        )
        df.columns = names
...
class NodeStratigraphy:
    ...
    def __repr__(self):
        return "NodeStratigraphy(node_id={}, gse={}, layer_thicknesses={})".format(
            self.node_id, self.gse, self.layer_thicknesses
        )
