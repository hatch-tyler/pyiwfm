...
def set_flag_to_echo_progress(flag):
    ...
    if flag != YES_ECHO_PROGRESS and flag != NO_ECHO_PROGRESS:
        set_last_message('flag to echo progress is not recognized', FATAL, THIS_PROCEDURE)
    else:
...
def set_default_message_destination(destination):
    ...
    '''
    test = (destination == SCREEN) or \
           (destination == FILE) or \
           (destination == SCREEN_FILE)
    if not test:
        set_last_message('message destination is not recognized!', FATAL, THIS_PROCEDURE)
...
def set_log_file_name(file_name):
    ...
    try:
        if log_file:
            set_last_message('Error in opening new log file! A log file is already created!', FATAL, THIS_PROCEDURE)
    except NameError:
...
def get_log_file_unit(file_name):
    raise NotImplementedError("This construct is not used in the" + \
                              "python implementation of IWFM")
...
def log_all_message_types(message_array, error_level, prog_name, destination):
    ...
    if error_level == MESSAGE:
        ...
        if will_print_to_file:
            ...
            log_file.write('*')
            log_file.write('* FATAL:')
            log_file.write('*   Incorrect error level returned from procedure {}'.format(prog_name))
            log_file.write('*   ({})'.format(prog_name))
            log_file.write('*')
            log_file.write('*******************************************************************************')
...
def log_message(message, error_level, prog_name, destination=default_message_destination):
...
def log_last_message(destination=default_message_destination):
    ...
    message_local = '{}{}*   ({}){}{}'.format(message_local, LINE_FEED, last_message_procedure, LINE_FEED, '*'*80)
...
def print_run_time(start_time=None, stop_time=None, destination=default_message_destination):
    ...
    if hours > 0:
        message_array[1] = "{}{} HOURS {} MINUTES {:6.3f} SECONDS".format(message_array[1], hours, minutes, seconds)
    elif minutes > 0:
        message_array[1] = "{}{} MINUTES {:6.3f} SECONDS".format(message_array[1], minutes, seconds)
    else:
        message_array[1] = "{}{:6.3f} SECONDS".format(message_array[1], seconds)
    
    if warnings_generated:
        if len(message_array) <= 2:
            message_array.append('WARNINGS/INFORMATIONAL MESSAGES ARE GENERATED!{}'.format(LINE_FEED))
        else:
            message_array[2] = 'WARNINGS/INFORMATIONAL MESSAGES ARE GENERATED!{}'.format(LINE_FEED)
        ...
        try:
            ...
            message_array[2] = "{}FOR DETAILS CHECK FILE '{}'.{}".format(message_array[2], log_file_name, LINE_FEED)
...
if __name__ == "__main__":
    ...
    log_message('Node number does not have any surrounding elements!', WARN, THIS_PROCEDURE)
    set_last_message('An application grid that is already defined is being re-defined!',FATAL,THIS_PROCEDURE)
    #print(warnings_generated)
    log_last_message()
    stop_time = program_timer.stop_timer()
