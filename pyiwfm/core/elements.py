...
class IWFMElements:
    ...
    def __init__(self, ne, nregn, rnames, elements):
        ...
        if not isinstance(rnames, list) and all(
            [isinstance(val, str) for val in rnames]
        ):
        ...
        if len(rnames) != nregn:
            raise ValueError(
                "There must be {} subregion names in rnames. {} provided".format(
                    nregn, len(rnames)
                )
            )
        ...
        if not isinstance(elements, (list, tuple)) and all(
        ...
        if len(elements) != ne:
            raise ValueError(
                "There must be {} elements. {} provided.".format(ne, len(elements))
            )
    ...
    def get_subregion_from_id(self, element_id):
        ...
        if not isinstance(element_id, int):
            raise TypeError(
                "id must be an integer. value provided is a {}".format(type(element_id))
            )
    ...
    def to_dataframe(self, filter_zeros=True):
        ...
        elements_dict = dict(
            ElementID=np.repeat(element_ids, num_nodes),
...
class Element:
    ...
    def __init__(self, element_id, node_ids, subregion):
        ...
        if not isinstance(node_ids, (list, tuple, np.ndarray)):
            ...
            if not all(
                [isinstance(val, (int, np.int32, np.int64)) for val in node_ids]
            ):
                raise TypeError(
                    "each node id in node_ids must be an integer: {}".format(node_ids)
                )
            ...
            if len(zero_index) > 1:
                raise ValueError(
                    "{} node ids were provided equal to 0. There can only be one".format(
                        len(zero_index)
                    )
                )
            elif len(zero_index) == 1 and zero_index[0] != 3:
                raise IndexError(
                    "A 0 can only be provided as the last value of node_ids"
                )
    ...
    def __repr__(self):
        return "Element(element_id={}, node_ids={}, subregion={})".format(
            self.element_id, self.node_ids, self.subregion
        )
    ...
    def from_string(cls, string):
        ...
        if len(string_list) != 6:
            raise ValueError(
                "string must include exactly 6 values for a groundwater node"
            )
