...
class IWFMHydrographReader:
    ...
    def _parse(self) -> None:
        ...
        for dline in data_lines:
            ...
            time_str = parts[0]
            iso_time = self._parse_iwfm_datetime(time_str)
            if iso_time is None:
                continue
    ...
    @staticmethod
    def _parse_iwfm_datetime(time_str: str) -> str | None:
        """Parse IWFM datetime format to ISO 8601 string.

        Handles formats like:
        - ``MM/DD/YYYY_HH:MM``
        - ``MM/DD/YYYY_24:00`` (end of day -> next day 00:00)
        """
        try:
            # Replace underscore with space for parsing
            cleaned = time_str.replace("_", " ")

            # Handle 24:00 convention (end of day)
            if "24:00" in cleaned:
            ...
            dt = datetime.strptime(cleaned, "%m/%d/%Y %H:%M")
            return dt.isoformat()
