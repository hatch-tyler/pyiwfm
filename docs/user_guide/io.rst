...
ASCII Mesh and Stratigraphy Files
---------------------------------
...
.. code-block:: python
    ...
    # Default output: IWFM-conventional ASCII-art title block,
    # FACTXY=1.0 line, AQT_L*/AQF_L* column legend.
    write_nodes("output/nodes.dat", nodes)

    # FACTXY conversion factor (e.g. write metres but record a foot factor)
    write_nodes("output/nodes_ft.dat", nodes, factor=0.3048)

    # Custom header replaces the title block; data section is unchanged.
    write_nodes(
        "output/nodes_custom.dat",
        nodes,
        header="Project: C2VSimFG\nGenerated by build_mesh.py",
    )

Reading and Writing Elements
~~~~~~~~~~~~~~~~~~~~~~~~~~~~

``read_elements`` returns a 3-tuple ``(elements, n_subregions, subregion_names)``;
``write_elements`` accepts the same data via keyword arguments and infers
``n_subregions`` from the element subregion IDs when omitted.
...
.. code-block:: python
    ...
    elements, n_subregions, subregion_names = read_elements("Preprocessor/Element.dat")

    # Round-trip with subregion names preserved
    write_elements(
        "output/elements.dat",
        elements,
    ...
    # Build elements programmatically (n_subregions inferred, names default
    # to "Region_1", "Region_2", ...).
    new_elements = {
        1: Element(id=1, vertices=(1, 2, 5, 4), subregion=1),
        2: Element(id=2, vertices=(2, 3, 6, 5), subregion=2),
    }
    write_elements("output/new_elements.dat", new_elements)

Triangles are written with a fourth vertex of ``0`` per IWFM convention.
...
.. code-block:: python
    ...
    # Default output emits the FACTEL=1.0 factor line and an
    # AQT_L1/AQF_L1/AQT_L2/AQF_L2/... column legend per layer.
    write_stratigraphy("output/stratigraphy.dat", stratigraphy)

    # FACTEL conversion factor (e.g. records elevations were converted
    # by the given factor on read-back).
    write_stratigraphy("output/stratigraphy_ft.dat", stratigraphy, factor=0.3048)

Writing All Three from a Model
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

If you have an :class:`~pyiwfm.core.model.IWFMModel`, the
``PreProcessorWriter`` orchestration class handles all three subfiles
plus the main control file in one call. It delegates to the same
``mesh.write_*`` functions shown above.
...
.. code-block:: python
    ...
    paths = writer.write_all()  # writes nodes, elements, stratigraphy, main
